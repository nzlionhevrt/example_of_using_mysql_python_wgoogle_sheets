...
### База данных: `analytics_test`

#### Таблица `sales_fact`
Фактические данные о продажах:
- `id` - первичный ключ
...
#### Таблица `revenue_plan`
План выручки из Google Sheets:
- `id` - первичный ключ
- `month` - месяц плана
- `club_id` - ID клуба
- `club_name` - название клуба
- `plan_revenue` - плановая выручка
- `created_at`, `updated_at` - временные метки
- Уникальный ключ по `(month, club_id)`

## Подключение к MySQL

Для подключения из внешних приложений (например, DataLens):

- **Хост**: `localhost` (или IP вашего сервера)
- **Порт**: `3306`
- **База данных**: `analytics_test`
- **Пользователь**: `analytics_user`
- **Пароль**: значение из `.env` (по умолчанию: `analytics_password`)
